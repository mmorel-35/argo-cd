...
func (r *reporter) PostMsgReceive(payload any, _ error, _ time.Duration) {
	logRequest(r.ctx, r.entry, r.info, payload, r.logClaims)
}

func reportable(entry *logrus.Entry, logClaims bool) interceptors.CommonReportableFunc {
	return func(ctx context.Context, c interceptors.CallMeta) (interceptors.Reporter, context.Context) {
		return &reporter{
			ctx:       ctx,
			entry:     entry.WithFields(ctx_logrus.Extract(ctx).Data),
			info:      fmt.Sprintf("received %s call %s", c.Typ, c.FullMethod()),
			logClaims: logClaims,
		}, ctx
	}
}
...
func PayloadStreamServerInterceptor(entry *logrus.Entry, logClaims bool, decider func(context.Context, interceptors.CallMeta) bool) grpc.StreamServerInterceptor {
	return selector.StreamServerInterceptor(interceptors.StreamServerInterceptor(reportable(entry, logClaims)), selector.MatchFunc(decider))
}
...
func PayloadUnaryServerInterceptor(entry *logrus.Entry, logClaims bool, decider func(context.Context, interceptors.CallMeta) bool) grpc.UnaryServerInterceptor {
	return selector.UnaryServerInterceptor(interceptors.UnaryServerInterceptor(reportable(entry, logClaims)), selector.MatchFunc(decider))
}
