...
func TestProcessHydrationQueueItem_ValidationFails(t *testing.T) {
	...
	d.EXPECT().PersistAppHydratorStatus(mock.Anything, mock.Anything).Run(func(orig *v1alpha1.Application, newStatus *v1alpha1.SourceHydratorStatus) {
		if orig.Name == app1.Name {
			persistedStatus1 = newStatus
		} else if orig.Name == app2.Name {
...
func TestHydrator_hydrate_Success(t *testing.T) {
	...
	rc.EXPECT().GetRevisionMetadata(mock.Anything, mock.Anything).Return(&v1alpha1.RevisionMetadata{Message: "metadata"}, nil).Run(func(ctx context.Context, in *repoclient.RepoServerRevisionMetadataRequest, opts ...grpc.CallOption) {
	...
	cc.EXPECT().CommitHydratedManifests(mock.Anything, mock.Anything).Return(&commitclient.CommitHydratedManifestsResponse{HydratedSha: "hydrated123"}, nil).Run(func(ctx context.Context, in *commitclient.CommitHydratedManifestsRequest, opts ...grpc.CallOption) {
