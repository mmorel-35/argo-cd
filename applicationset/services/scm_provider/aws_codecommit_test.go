	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			for _, repo := range testCase.repositories {
				...
				codeCommitClient.EXPECT().GetRepositoryWithContext(ctx, &codecommit.GetRepositoryInput{RepositoryName: aws.String(repo.name)}).
			...
			if testCase.expectListAtCodeCommit {
				codeCommitClient.EXPECT().ListRepositoriesWithContext(ctx, &codecommit.ListRepositoriesInput{}).
					Return(&codecommit.ListRepositoriesOutput{Repositories: codecommitRepoNameIdPairs}, testCase.listRepositoryError)
			} else {
				taggingClient.EXPECT().GetResourcesWithContext(ctx, mock.MatchedBy(equalIgnoringTagFilterOrder(&resourcegroupstaggingapi.GetResourcesInput{
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.expectedGetFolderPath != "" {
				codeCommitClient.EXPECT().GetFolderWithContext(ctx, &codecommit.GetFolderInput{
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.allBranches {
				codeCommitClient.EXPECT().ListBranchesWithContext(ctx, &codecommit.ListBranchesInput{
					RepositoryName: aws.String(name),
				}).
					Return(&codecommit.ListBranchesOutput{Branches: aws.StringSlice(testCase.branches)}, testCase.apiError)
			} else {
				codeCommitClient.EXPECT().GetRepositoryWithContext(ctx, &codecommit.GetRepositoryInput{RepositoryName: aws.String(name)}).
