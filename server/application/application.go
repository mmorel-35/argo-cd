...
func (s *Server) updateApp(ctx context.Context, app *appv1.Application, newApp *appv1.Application, merge bool) (*appv1.Application, error) {
	for i := 0; i < 10; i++ {
...
func (s *Server) logAppEvent(ctx context.Context, a *appv1.Application, reason string, action string) {
...
func (s *Server) logResourceEvent(ctx context.Context, res *appv1.ResourceNode, reason string, action string) {
	eventInfo := argo.EventInfo{Type: corev1.EventTypeNormal, Reason: reason}
	user := session.Username(ctx)
	if user == "" {
		user = "Unknown user"
