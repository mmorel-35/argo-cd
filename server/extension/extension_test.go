...
func TestCallExtension(t *testing.T) {
	...
	withMetrics := func(f *fixture) {
		f.metricsMock.EXPECT().IncExtensionRequestCounter", mock.Anything, mock.Anything)
		f.metricsMock.EXPECT().ObserveExtensionRequestDuration", mock.Anything, mock.Anything)
	}
	...
	withRbac := func(f *fixture, allowApp, allowExt bool) {
		...
		f.rbacMock.EXPECT().EnforceErr", mock.Anything, rbac.ResourceApplications, rbac.ActionGet, mock.Anything).Return(appAccessError)
		f.rbacMock.EXPECT().EnforceErr", mock.Anything, rbac.ResourceExtensions, rbac.ActionInvoke, mock.Anything).Return(extAccessError)
	}

	withUser := func(f *fixture, userId string, username string, groups []string) {
		f.userMock.EXPECT().GetUserId", mock.Anything).Return(userId)
		f.userMock.EXPECT().GetUsername", mock.Anything).Return(username)
		f.userMock.EXPECT().GetGroups", mock.Anything).Return(groups)
	}
